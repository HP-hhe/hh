...
(function($){
	...
	$.confirm = function(params){
		...
		$(markup).hide().appendTo('body').fadeIn();//隐藏
...
$(document).ready(function (){
	...
	menu();
	//登录表单的验证
	$('#username').bind('blur', Checkusername);//将事件和函数绑定到元素
	...
	$("#adetail").fancybox();
	$(".stretch img").hover(function(){//选择浮在上面的元素
		if($(this).attr('src') == 'images/zd.png')//改变当前对象的属性
...
//获取新增的资产，使用post的请求方式；使用异步请求的方式
function getnew(){
	$.post('getnew',function(data){//请求的处理函数以及请求成功所需要执行的函数
			var json = eval('(' + data + ')');//对获取的数据进行解析
			$('#getnew').empty();
			$.each(json,function(idx,item){ //使用each方法来进行遍历
				if(idx == json.length)
					return true;
				//获取出来的数据插入到页面当中
...
function login(){
	...
	//采用post的请求方式，login方法是请求的url；后面为用户名和密码
